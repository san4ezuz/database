...
--Вывести количество призывников по социальным статусам за заданный период 
 CREATE VIEW Socstatus AS
	SELECT status.status_id, socstate.dateto, socstate.datefrom from status, socstate
	where status.socst_id = socstate.socst_id and socstate.datefrom <= '01.01.2015' AND socstate.dateto >= '01.01.2016';
	SELECT COUNT(*) FROM Socstatus;
